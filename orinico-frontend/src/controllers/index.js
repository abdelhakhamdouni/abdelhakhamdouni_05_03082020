...
let data_from_api = dataHandler.fetchData('http://localhost:3000/api/teddies')
    ...
    data.forEach(element => {
        ...
        content += `
                    <div class="card">
                        ...
                        <p class="card-text"><strong>${toEuro(element.price)}€</strong></p>
                        <p class="card-text"><small class="text-muted">disponible en :  `
                        
        element.colors.forEach(color=>{
            let color_ = ""
            if( color === "Dark brown" ){color_ = "#4c1414"}
            else if( color === "Pale brown" ){color_ = "#c88e62" }
            else{color_ = color}
